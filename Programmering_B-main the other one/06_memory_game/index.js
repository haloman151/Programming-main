...
function setupGame(){
    ...
    images.map(i => {
        ...
        //Opret spillekort, læg dem ind i game containeren og put et billede ind i div'erne
        var card = createElement('div').addClass('card').attribute('img-source', i).parent
        (container).child(createImg(i)).mousePressed(()=>{
            if(flippedCards.length < 2){
                card.addClass('show')
                flippedCards.push(card)
                //Hvis der er to kort i flippedCards, skal vi tjekke 2
                if(flippedCards.length == 2){
