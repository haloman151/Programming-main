//This scrupt takes a csv file and clean the data into a javascript array

var table
//cleanData will hold the javascript objects we intend to use
var cleanData = []

const csvFile = './assets/BirdSpecies_dataset.csv'
//vi vil kun bruge max tusind rækker - da vi skal tegne dem på skærmen
const maxRows = 1000

function preload(){
    //load table er en p5 funktion der henter en tabel fra en fil
    table = loadTable(csvFile, 'csv', 'header')
    console.log('Data tabel loaded')
}

//Kan jeg lave en algoritme som kan forudsige fugles art udfra deres kropslængde og Vingefang?
function setup(){
    console.log("Rå data kolonner: ", table.columns)
    var xValue = "BodyLength"
    var yValue = "Wingspan"
    var labelValue = "SpeciesClass"
    ...
    cleanData = cleanData.filter( row => {
        ...
        //Men vi skal også tjekke om label er noget HVIS vi har en labet´
    ...
    select('#status').html(`Vi har nu renset data og skåret det ned til max 1000 rækker - kig i konsollen <3`)
