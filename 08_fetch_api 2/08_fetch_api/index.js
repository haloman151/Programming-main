...
function setup(){
    ...
    select('#playerHitBtn').mousePressed(()=>drawCard("dealer"))
...
async function drawCard(newState){
    ...
    if(state == "begin"){
        ...
        //først ligger vi kortedes værdi oven i spiller variablen (unden hensyn til ES)
        ...
        //Nu er vi en situation hvor spillere faktisk kunne have vundet, kunne have 22 (to es'er), eller vare har fået et eller andet tal under 21
        if(player.total ==22){
            player.total = 12
        }
    ...
    if(state == "playerLose"){
        //Vis noget på skærmen hvor det er tydeligt at spilleren har tabt
        //hvis restart knap
    ...
    if(state == "player"){
        console.log('showtime - næste time skal vi impletermere')
...
function showCards(){
    console.log("ShowCards er klar med: ", "player cards:", player.cards,"dealer cards:", dealer.cards)
